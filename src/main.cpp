...
// Copyright (c) 2016-2017 The GreenShilling developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20);//32
static CBigNum bnInitialHashTarget(~uint256(0) >> 20);//40
unsigned int nStakeMinAge = STAKE_MIN_AGE;
int nCoinbaseMaturity = COINBASE_MATURITY_GSL;
...
const string strMessageMagic = "GreenShilling Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // GreenShilling: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // GreenShilling: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // GreenShilling: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// GreenShilling: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // GreenShilling: subsidy is cut in half every 16x multiply of difficulty
...
// GreenShilling: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge)
{
    static int64 nRewardCoinYear = CENT * 3;  // 3% creation amount per coin-year
...
// GreenShilling: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // GreenShilling: target change every block
    // GreenShilling: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // GreenShilling: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // GreenShilling: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // GreenShilling: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // GreenShilling: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // GreenShilling: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // GreenShilling: track money supply and mint amount info
    ...
    // GreenShilling: fees are not collected by miners as in bitcoin
    // GreenShilling: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// GreenShilling: total coin age spent in transaction, in the unit of coin-days.
...
// GreenShilling: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // GreenShilling: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // GreenShilling: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // GreenShilling: record proof-of-stake hash value
    ...
    // GreenShilling: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // GreenShilling: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // GreenShilling: coinbase output should be empty if proof-of-stake block
    ...
    // GreenShilling: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // GreenShilling: check that the block satisfies synchronized checkpoint
    ...
    // GreenShilling: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // GreenShilling: check proof-of-stake
    ...
    // GreenShilling: verify hash target and signature of coinstake tx
    ...
    // GreenShilling: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // GreenShilling: getblocks may not obtain the ancestor block rejected
    ...
    // GreenShilling: if responsible for sync-checkpoint send it
...
// GreenShilling: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "GreenShilling", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        bnInitialHashTarget = CBigNum(~uint256(0) >> 17);
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "GreenShilling", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Oodles is making new coin Date: 06/04/2017";
        CTransaction txNew;
        txNew.nTime = 1497356710;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x0ed05839cb6eda5c3d873e1deea880ce9c3d1a1afd414e86dc03c7af2192b74a"));
        ...
        // GreenShilling: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // GreenShilling: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // GreenShilling: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // GreenShilling: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // GreenShilling: should not enter safe mode for longer invalid chain
    // GreenShilling: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 220) && !fTestNet)
    {
        nPriority = 100;
        strStatusBar = "WARNING: Checkpoint is too old. Wait for block chain to download, or notify developers of the issue.";
    }

    // GreenShilling: if detected invalid checkpoint enter safe mode
    ...
    // GreenShilling: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // GreenShilling: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // GreenShilling: record my external IP reported by peer
        ...
        // GreenShilling: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // GreenShilling: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // GreenShilling: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // GreenShilling: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // GreenShilling: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // GreenShilling: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
