...
// Copyright (c) 2011-2015 The GreenShilling developers
...
{
    // Wait for IRC to get it first - disabled with GreenShilling
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "GreenShilling " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are GreenShilling dns seeds.
static const char *strDNSSeed[][2] = {
    {"seed", "greenshilling.org"},
    {"seedgsc", "greenshilling.info"},
    // {"7server", "gscseed.ns.7server.net"},
    // {"altcointech", "dnsseed.gsc.altcointech.net"},
    // {"diandianbi", "seed.diandianbi.org"},
    // {"tnseed", "tnseed.GreenShilling.net"},
    // {"tnseedgsc", "tnseedgsc.GreenShilling.net"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // GreenShilling enables dns seeding with testnet too
    {
...
// GreenShilling: stake minter thread
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  GreenShilling is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // IRC disabled with GreenShilling
    ...
    // GreenShilling: mint proof-of-stake blocks in the background
